...
class CCmdSet():
    ...
    def getCmdSet(self):
        ...
        buffer = struct.pack('I I I %ds'%DataLength, DISCRIMINATOR, VERSION, DataLength,self.doc.toxml(encoding="euc-kr"))
        return buffer
        ...
        if bodyTag.childNodes.length > 0:
            ...
            if paramList.length > 0:
                for item in paramList:
                    nameTag = item.getElementsByTagName('name')
                    typeTag = item.getElementsByTagName('type')
                    valueTag = item.getElementsByTagName('value')
                    ...
                    if len(self.m_param) == 0:
                        ...
                        i = 0
                        appendFlag = False
                        while i < (len(self.m_params)):
                            if self.m_params[i]['name'] == param['name']:
                                self.m_params[i]['type'] = param['type']
                                self.m_params[i]['value'] = param['value']
                                break;
                            i+=1
                        if i == len(self.m_params):  
                            self.m_params.append(param)
                    """
